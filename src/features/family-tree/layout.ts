...
export const calculateOptimalPosition = (
	...
	switch (relationship) {
		...
		case 'father':
		case 'mother': {
			const existingParents = existingNodes.filter(node => Number(node.data?.generation) === newGeneration && ['father', 'mother'].includes(node.data?.relationship as string));
			let parentOffset;
		...
		case 'daughter': {
			...
			const existingChildren = existingNodes.filter(node => node.data?.generation === newGeneration && ['son', 'daughter'].includes(node.data?.relationship as string));
			const childrenCount = existingChildren.length;
			let childOffset = 0;
			if (childrenCount === 0) childOffset = 0; else {
